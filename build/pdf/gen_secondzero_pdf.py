...
src = pathlib.Path("/home/claude/wetheusers/docs/essay/the-second-zero.md").read_text()
parts = re.split(r'\n-{3,}\n', src, maxsplit=1)
...
<section class="docheader">
  ...
  <div class="title">The Second Zero</div>
  <div class="drule"></div>
  <div class="subtitle">Where the road leads &mdash; the same internet, owned by its citizens, and how the money works</div>
...
html = TEMPLATE.replace("__CSS__", CSS).replace("__BODY__", html_body).replace("__META__", dm.meta_html("the-second-zero"))
pathlib.Path("/home/claude/secondzero.html").write_text(html)
